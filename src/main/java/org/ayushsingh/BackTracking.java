...
public class BackTracking {
    ...
    public static void subset2(int i, String input, StringBuilder curr, List<String> result) {
        ...
        for (int j = i; j < input.length(); j++) {
            ...
            if (j > i && input.charAt(j - 1) == input.charAt(j)) { // this will skip the duplicates, Note: Input MUST BE
                                                                   // SORTED
                continue;
            }
    ...
    public static void permutations(String input, StringBuilder curr, List<String> result, boolean[] visited) {
        ...
        for (int i = 0; i < input.length(); i++) {
            ...
            // We skip when !used[i - 1] because it means the previous identical character
            // has not been chosen in the current permutation prefix.
            // If we allow the current duplicate to be chosen in that situation, we would
            // start a new permutation branch that is identical to one already explored,
            // causing duplicate permutations.
            if (i > 0 && input.charAt(i - 1) == input.charAt(i) && !visited[i - 1])
                continue; // Only WORKS if the Input is SORTED
    ...
    public static void combinationSum(int start, int[] nums, int target, List<Integer> curr,
    ...
    public static void Nqueens(int start, int n, int[][] board, boolean[] colUsed, boolean[] mainD, boolean[] antiD,
            List<int[]> result) {
        ...
        for (int i = 0; i < n; i++) {
            if (colUsed[i] || antiD[start + i] || mainD[start - i + (n - 1)])
                continue;
    ...
    // When generating permutations with duplicates, I sort the input and ensure
    // that duplicate elements are only chosen in a specific order.
    // I skip a duplicate if its previous identical element has not been used in the
    // current path, which prevents duplicate sibling branches.
